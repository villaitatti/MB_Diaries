...
2. **clean vectors** — `_clean_vectors()` splits page-marker tokens (e.g. `[0255]`) out of runs into their own paragraphs so markers become standalone delimiters.
3. **vectors → pages** — `parse_pages()` walks paragraphs *in reverse*, treating each line that exactly matches the page-marker regex as a page boundary, and groups the preceding paragraphs into that page. Produces an `OrderedDict[int, {text, content:[paragraph]}]`. Note `const.key_paragraphs == "content"`.
4. **fixups** — `_fix_missing_whitespace()` inserts spaces after punctuation glued to the next word (skipping decimals); writes `missing_whitespace_report.log` for manual review.
...
`pages.json`, `vectors.json` (intermediate JSON), `txt/<page>.txt` (per-page plain text — `rdf.py` reads these back for fulltext), `html/<diary>_<page>.html` (per-page styled HTML, also mirrored into the external app dir, see below), `ttl/{diary,document,annotation}/*.ttl` (the RDF graphs), plus `error.log` and the whitespace report. With `--concatenate`: `<diary>_concatenated.txt`, `<diary>_concatenated_with_conts.txt` (marks page continuations / mid-page date headers), and `<diary>_stats.json`.
