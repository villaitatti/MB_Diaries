...
def create_diary_graph(diary_number, image, title, index, iiif_manifest):
  ...
  if iiif_manifest:
    g.add((BASE_NODE, MB_DIARIES['iiif-manifest'],
          Literal(iiif_manifest, datatype=XSD.string)))
  ...
  g.namespace_manager.bind(
    'mbdiaries-ontology', MB_DIARIES, override=True, replace=True)
...
def write_page_graph(filename, diary, day, output_path):
  ...
  writer.create_dir(os.path.dirname(os.path.abspath(filename)))
  g = create_page_graph(
    diary, day['page'], day['day'], day['image'], output_path)
...
def create_page_graph(diary_number, page_number, page, image, output):
  ...
  g.add((PAGE_NODE, MB_DIARIES['index'], Literal(
    page_number, datatype=XSD.integer)))
...
def create_day_graph(diary_number, day):
  ...
  g.add((DAY_NODE, CRM.P46i_forms_part_of,
        URIRef(f'{RESOURCE}diary/{diary_number}')))
  g.add((DAY_NODE, MB_DIARIES['order'], Literal(
    day_index, datatype=XSD.integer)))
